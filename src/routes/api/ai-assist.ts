...
Max 6 suggestions. Skip leads that already have an open task.`;
  ...
  if (action === "ask_crm") {
    return `You are an assistant for a solar CRM ("Ask your CRM"). The user asks a natural-language question about their business data. A JSON snapshot of the CRM is provided (leads, tasks, visits, quotes, invoices, team, settings, inventory, stockMoves, expenses). "inventory" is the product catalog with current stock, minStock and status (ok/low/out). "stockMoves" records IN (purchase), OUT (used at a site — leadId points to the site/lead) and adjust movements. "expenses" tracks daily team spending — each entry has desc, category, amount, date, memberId (team member who paid), leadId (site the expense is linked to), method (Cash/Bank/etc.), vendor, and hasReceipt (whether a bill photo is attached). Use this data to answer questions about stock levels, product usage per site, total/site-wise expenses, spending by team member or category, and profitability.
